...
// Performs K-means clustering on this data
...
fn main() -> Result<()> {
    ...
    let result = kmeans_clustering(&data, n_clusters, max_iterations, tolerance, seed)?;
    
    println!("========= K-means Clustering Report =========");
    ...
    let root = BitMapBackend::new("kmeans_demo.png", (800, 600)).into_drawing_area();
    root.fill(&WHITE)?;
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption("K-means Clustering Demo", ("sans-serif", 30))
        .margin(10)
    ...
    for (point_idx, &cluster_id) in result.assignments.iter().enumerate() {
        // K-means always assigns points to clusters, so we don't need to check for outliers
    ...
    root.present()?;
    println!("Output saved as kmeans_demo.png");
